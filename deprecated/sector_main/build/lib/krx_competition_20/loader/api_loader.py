...
class SECTOR_LOADER:
    """
    SECTOR_LOADER : symbol : sector_code 데이터를 로드하는 클래스
    """
    def __init__(self, symbols) -> None:
        """
        SECTOR_LOADER의 생성자 
        
        :param list symbols: sector_code를 추출할 symbol들의 리스트
        """
        self.symbols = symbols

    @staticmethod
    def load_sector_info_dict(symbol):
        """
        sector_code를 추출하는 메서드
        :param list symbols: sector_code를 추출할 symbol들의 리스트
        :return: symbol을 넣으면 sector_code를 주는 dictionary
        :rtype: dict
        """
        try:
            ...
            return sector_info_dict
        except:
            return None

    def __call__(self):
        """
        SECTOR_LOADER의 파이프라인을 제공하는 메서드
        
        :return: symbol과 sector를 가지고 있는 dataframe
        :rtype: pd.DataFrame
        """
        sector_info_list = list()
        symbols = self.symbols
        for symbol in symbols:
            sector_info_dict = self.load_sector_info_dict(symbol)
            if sector_info_dict:
                sector_info_list.append(sector_info_dict)
        sector_info_df = pd.DataFrame(sector_info_list)
